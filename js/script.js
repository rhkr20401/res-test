let funcObj = {
  ...
  f_1:function(){
    ...
    //숫자 카운팅 - 자바스크립트 내에서 제이쿼리 셋팅함
    //.count 클래스의 요소가 data-count속성에 설정된 숫자까지 3초동안 점점 숫자가 증가하는 카운트 애니메이션
    //대상.animate({속성:값, 속성:값,},시간,이징,끝나면 할 일);
    $(function(){
      $(".count").each(function(){ //각각의 요소들을 반복적으로 순회(자바스크립트의 forEach와 동일)
        let crt = $(this); //각각의 요소
        let countTo = crt.attr('data-count');

        //초기숫자를 객체로 생성(현재요소의 텍스트 내용)->이 객체의 countNum 속성이 애니메이션 대상
        $({countNum:crt.text()}).animate({
          countNum:countTo//countNum 속성을 countTo 값까지 애니메이션
        },{
          duration:3000,
          easing:'linear',
          step:function(){//애니메이션 중 매 스탭(step)마다 실행
            crt.text(Math.floor(this.countNum))
          },
          complete:function(){//애니메이션이 끝난 후 실행
...
window.addEventListener('scroll',function(){
  ...
  let outerHeight = this.window.outerHeight;//브라우저의 높이값(border와 padding을 포함한 높이값)

  //인덱스 i는 forEach((element,index)=>{})의 두번째 인자로 받아옴
  section.forEach((sec,i)=>{
    if(scroll > section[i].offsetTop - outerHeight && scroll < section[i].offsetTop - outerHeight + section[i].offsetHeight){
      funcObj['f_'+i]();
      console.log(i);
    }
  });
...
/* 
스크롤 섹션의 offsetTop에 정확히 맞을때만 감지하면 너무 늦음
그래서 outerHeight만큼 미리 앞당겨서 인식 + 뷰포트에 다 들어오기 전에 이미 동작
현재 스크롤 위치가 "섹션이 뷰포트에 등장하기 시작할 시점"과 "센션이 뷰포트를 벗어나기 직전 시점" 사이에 있는지, 그때마다 섹션별 함수 f_i()를 실행 
*/



//JQuery______________________start
$(function(){
  const BODY = $("body");
...
//JQuery______________________end